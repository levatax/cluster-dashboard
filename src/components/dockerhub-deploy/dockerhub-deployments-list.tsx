    ...
    <div className="group flex items-center gap-3 px-3 py-2.5 transition-colors hover:bg-muted/40">
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className="mt-0.5 flex items-center gap-1.5 text-[11px] text-muted-foreground">
          <span className="truncate font-mono">{fullImage}</span>
          <a
            href={hubUrl}
            target="_blank"
...
export function DockerhubDeploymentsList({
  deployments,
  onRemove,
}: DockerhubDeploymentsListProps) {
  // Group by namespace
  const grouped: Record<string, DockerhubDeploymentRow[]> = {};
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-sm">
        <span className="font-medium">{deployments.length} deployment{deployments.length !== 1 ? "s" : ""}</span>
        {deployedCount > 0 && (
          <>
